...
class TestKeycloakRequester:
    ...
    def test_get_token_success(self, requests_mock):
        ...
        requester = KeycloakRequester(
            self.base_url, self.token_url, "user", "password", ""
        )
    ...
    def test_get_children(self, requests_mock):
        ...
        url: str = (
            f"{self.base_url}/groups/429c803a-a033-4e1e-8aea-73b92fd43003/children"
        )
        ...
        result = requester.query(
            "/groups/429c803a-a033-4e1e-8aea-73b92fd43003/children"
        )
        assert len(result) == 2
...
class TestKubespawnerKeycloak:
    ...
    async def test_get_groups(self, requests_mock):
        ...
        keycloak = KubespawnerKeycloak(
            ...
            environments_config=self.get_environments_config(),
    ...
    async def test_get_child_group(self, requests_mock):
        ...
        assert 2 == len(result)
        assert (
            "/jupyter-workspaces/colorectal cancer research group workspace"
            in result.keys()
        )
        assert "/jupyter-workspaces/default generic workspace" in result.keys()
        assert (
            "79cdf13c-a6bc-46cd-8a5d-1281b0fe8e53"
            == result[
                "/jupyter-workspaces/colorectal cancer research group workspace"
            ].id
        )
    ...
    async def test_get_workspaces(self, requests_mock):
        ...
        environments_config["jupyter_advanced"]["image"] = (
            "jupyter/datascience-notebook:latest"
        )
        environments_config["jupyter_default"] = {}
        environments_config["jupyter_default"]["image"] = (
            "jupyter/datascience-notebook:latest"
        )
        ...
        assert "Colorectal Cancer Research Group Workspace" == permitted_workspaces[
            0
        ].get("display_name")
        assert (
            "colorectal-cancer-research-group-workspace"
            == permitted_workspaces[0]["kubespawner_override"]["extra_labels"][
                "workspace"
            ]
        )
